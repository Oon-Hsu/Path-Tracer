...
{
	Colour *pBuffer;
	//Select between ray tracing or path tracing. Crashes if DoRayTrace and DoPathTrace are called at the same time
	if (m_pRayTracer->m_traceflag != RayTracer::TRACE_NULL)
	{
...
BOOL AppWindow::KeyUp(WPARAM key)
{
	...
	{
	case VK_F1:
		m_pPathTracer->m_traceflag = PathTracer::TRACE_NULL;
	...
	case VK_F6:
		...
		break;
	case VK_F7:
		m_pRayTracer->m_traceflag = RayTracer::TRACE_NULL;
		m_pPathTracer->m_traceflag = (PathTracer::TraceFlags)(PathTracer::TRACE_PATH);
		break;
	case VK_F8:
		m_pRayTracer->m_traceflag = RayTracer::TRACE_NULL;
		m_pPathTracer->m_traceflag = (PathTracer::TraceFlags)(PathTracer::TRACE_PATH | PathTracer::TRACE_GLOSS);
		break;
	default:
		break;
	}
